...
@include('common.partial.detail-property', ['name' => 'Submitted', 'value' => $user->partner_profile->submit ? 'Yes' : 'No'])
<form class="control" role="form" method="POST" action="{{ url('/admin/partner/' . $user->id) }}">
  {{ csrf_field() }}
  {{ method_field('PUT') }}
  <input type="hidden" name="submit" value="{{ $user->partner_profile->submit ? 0 : 1}}">
...
@include('common.partial.detail-property', ['name' => 'Approved', 'value' => $user->partner_profile->approval ? 'Yes' : 'No'])
<form class="control" role="form" method="POST"  action="{{ url('/admin/judges/' . $user->id) }}">
  {{ csrf_field() }}
  {{ method_field('PUT') }}
  <input type="hidden" name="approval" value="{{ $user->partner_profile->approval ? 0 : 1 }}">
  <div class="field is-horizontal">
    <div class="field-label is-normal">
    </div>
    <div class="field-body">
