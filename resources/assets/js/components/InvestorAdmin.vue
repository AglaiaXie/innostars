  ...
  <section>
    ...
    <b-table
      ...
      <template slot-scope="props">
        ...
        <b-table-column field="investor_profile.company" label="Company公司名">
          {{ props.row.investor_profile.company_name }}
        </b-table-column>

        <b-table-column field="investor_profile.title" label="Position职位">
        ...
        <b-table-column label="Status" v-if="permission.private">
          <span
            class="icon"
            v-bind:class="props.row.investor_profile.submit ? 'has-text-success' : 'has-text-warning'"
            title="Submit Status">
            <i class="fa fa-upload"></i>
          </span>
          <span
            class="icon"
            v-bind:class="props.row.investor_profile.approval ? 'has-text-success' : 'has-text-warning'"
    ...
    <div v-if="permission.export">
      <a class="button is-primary"
         :href="'/api/v1/investors/download?keyword=' + searchKeyword + '&industry=' + searchIndustry + '&competition=' + searchCompetition + '&submitted=' + searchSubmitted + '&approved=' + searchApproved">
        Download CSV
      </a>
    </div>
  </section>
...
<script>
    ...
    export default {
        props: ['permission'],
        components: {InvestorDetail, MessageCreate},
        methods: {
            index() {
                this.loading = true;
                axios.get('/api/v1/investors', {
                    params: {
            ...
            marsquerade(id, name) {
                this.$dialog.confirm({
                    message: 'Are you sure that you want to masquerade ' + name + '\'s investor account?',
                    onConfirm: function () {
            ...
            deleteAccount(id, name) {
                ...
                instance.$dialog.confirm({
                    title: 'Deleting investor account',
                    message: 'Are you sure that you want to delete <br/> ' + name + '\'s investor account?',
                    confirmText: 'Delete',
                    type: 'is-danger',
                    hasIcon: true,
                    onConfirm: function () {
                        axios.delete('/api/v1/investors/' + id)
                            .then(function () {
                                instance.$toast.open({
                                    message: 'Investor account deleted!',
                                    type: 'is-success'
                                });
                                instance.search();
                            })
